...
``` r
#histogram is a quick way to see the distribution of one variable
hist(hfi_2016$pf_score)
```

![](activity02_files/figure-gfm/distribution-plots-1.png)<!-- -->

``` r
hist(hfi_2016$pf_expression_control)
```
...
``` r
ggplot(hfi_2016, aes(y = pf_score, x = pf_expression_control)) +
  geom_point()
```
...
Answer: yes there is a very clear positive linear relationship with only
a few outliers. \#### Challenge
...
``` r
cor(hfi_2016$pf_score,hfi_2016$pf_expression_control,method="pearson")
```
...
- In the code chunk below titled `parsnip-spec`, replace “r” with “r”
...
``` r
 # rf_workflow <-
 #   workflow() %>%
 #   add_formula(pf_score ~ pf_expression_control) %>%
 #   add_model(lm_spec)
...
- In the code chunk below titled `glance-lm`, replace “r” with “r” just
  before the code chunk title.
...
7.  What is the value of $R^2$ for this model? 0.714132
...
```
    ...
    ##   term                  estimate std.error statistic  p.value
    ##   <chr>                    <dbl>     <dbl>     <dbl>    <dbl>
    ## 1 (Intercept)              4.24     0.167       25.4 1.32e-51
    ## 2 pf_expression_control    0.544    0.0301      18.1 6.54e-37
...
$\hat{\texttt{pf\score}} = 4.236628 + 0.54382 \times \texttt{pf\_expression\_control}$
...
``` r
test_aug <- augment(slr_train, new_data = test)
test_aug
```
    ...
    ##  1  6.00 -0.701   2016 BGD      Bangladesh South…              2.33         3.71
    ##  2  6.55  0.659   2016 BOL      Bolivia    Latin…              3.70         3.36
    ##  3  6.55  1.31    2016 BIH      Bosnia an… Easte…              6.31         5.00
    ##  4  6.68 -0.0179  2016 BRA      Brazil     Latin…              4.97         5.29
    ##  5  8.45 -0.465   2016 CPV      Cape Verde Sub-S…             NA           NA   
    ##  6  7.91  0.309   2016 CHL      Chile      Latin…              7.71         6.29
    ##  7  7.36  1.09    2016 HRV      Croatia    Easte…              6.31         5.73
    ##  8  8.86  0.466   2016 DNK      Denmark    Weste…              9.27         8.62
    ##  9  7.23 -0.310   2016 SLV      El Salvad… Latin…              5.11         4.98
    ## 10  6.82  0.757   2016 GEO      Georgia    Cauca…              5.65         5.43
    ## # ℹ 23 more rows
...
``` r
cor(test_aug$pf_score, test_aug$.pred)^2
```

    ## [1] 0.6997243

``` r
rmse(test_aug$pf_score, test_aug$.pred)
```
...
``` r
train_aug <- broom::augment(slr_train,test)
...
![](activity02_files/figure-gfm/fitted-residual1-1.png)<!-- -->
